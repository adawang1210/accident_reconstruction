...
- 從 **Overpass API** 抓 `origin_latlon` 周圍 `radius`（預設 250 m）內的 `building` way。
...
- **中心**：`origin_latlon`（重建檔的場景原點）。範例 `sample_intersection` = `25.0413, 121.532`
  → 台北市大安區忠孝東路三段路口。
- **抓取範圍**：Overpass `way["building"](around:${radius},lat,lon)`，`radius` 預設 **80 m**
  → 以路口為圓心、**半徑 80 m（直徑約 160 m）** 的圓形區域內所有建物 way。
- **投影**：本地等距投影（以原點為切平面）
  `east = (lon − lon0)·111320·cos(lat0)`、`north = (lat − lat0)·111320`，
  場景座標 `x = east`、`z = −north` → 與重建（道路 / 車輛 / 撞擊點）**同框對齊**，不需額外轉換。
- **可視 vs 抓取**：抓取是 80 m 半徑；實際入鏡範圍再由相機開場角度與 `fog`（`150 → 360 m`）
  收成「車禍場景大小」。
- **改顯示區域**：調 `CityBlocks` 的 `radius`（由 `Scene.tsx` 傳入）。越大脈絡越多、越吃 Overpass
  流量；範例路口 250 m 半徑約 **293 棟**，80 m 半徑只剩路口周邊數十棟。
...
- **街道**：淺色路面 + 每條 `data.roads` 中心線擠成柏油路帶，加**白色路緣線 + 黃色虛線車道線**（地面標示）。
- **淺色日間主題**：`useSchematic` 時背景/霧用淺色 `#e9edf3`，遠處柔和淡出。
- **範圍貼合車禍場景**：建物半徑預設 **80 m**、相機開場收成貼近路口的俯瞰（約 65° 下視，避免高樓擋住路口）、霧 150→360 m。

可調參數：`CityBlocks` 的 `radius`；`SchematicStreets` 的 `ROAD_WIDTH`／顏色／`Y_*` 疊放高度；
`Scene.tsx` 開場運鏡座標與 `fog`。
...
| 狀態 | 意義 | 由誰設定 |
|---|---|---|
| `groundY` | 把內容群組／相機抬到實際街面高度 | `GoogleTiles.onGround`（已 clamp `|y|<1000`） |
...
- 啟動：
  ...
  ```
- `reconstruction.json` 來源：預設 `public/reconstruction.sample.json`，可用
